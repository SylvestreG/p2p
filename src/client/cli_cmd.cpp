...
void p2p_send_msg(std::shared_ptr<central_client> client,
                  std::string const &name, replxx::Replxx &rx,
  ...
  std::string const &data{input.substr(
      data_pos, input.size() - data_pos)}; //  for space after addr and \n
...
// clear screen
void clear(replxx::Replxx &rx, std::string const &input) { rx.clear_screen(); }
