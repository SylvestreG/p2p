...
replxx::Replxx::completions_t hook_completion(std::string const& context, int& contextLen, std::vector<std::string> const& examples) {
...
void cli::add_command(std::string const& command_name, command_fn fn) {
...
void cli::run() {
  ...
  _rx.set_completion_callback( std::bind( &hook_completion, std::placeholders::_1, std::placeholders::_2, cref( _keywords ) ) );
