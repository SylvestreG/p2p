...
Status central_server_server::client_register(ServerContext* context,
                                              client_information const* request,
                                              bool_msg *reply) {
  std::cout << "registering client: " << request->id().name() << " (" <<
    request->addr() << ":" << request->port() << std::endl;
  return Status::OK;
};

Status central_server_server::client_lookup(ServerContext* context,
                                            client_id const* request,
                                            client_info_response *reply) {
  return Status::OK;
};